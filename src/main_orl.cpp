...
int main(int argc, char *argv[]){
    ...
    string fn_csv = "/Users/calvinlee/eigenfaces/data/at.txt";
    try {
    ...
    vector<int> testlabels;
        string fn_csv_test = "/Users/calvinlee/eigenfaces/data/at_test.txt";
    try {
        read_csv(fn_csv_test, testimages, testlabels);
    } catch(exception& e) {
        cerr << "Error opening file \"" << fn_csv_test << "\"." << endl;
        exit(1);
    }
    Mat col1 = trainimages[0].reshape(0, trainimages[0].rows);
    Mat tempmat(trainimages[0].rows, trainimages[0].cols, CV_8UC1);
    olbp(col1, tempmat);
    imshow("1", tempmat);
    ...
    // Mat col1 = trainimages[0].reshape(0, trainimages[0].rows);
    // Mat lbpres(trainimages[0].rows, trainimages[0].cols, CV_8UC1);
    // olbp(col1, lbpres);
    // imshow("Original image", col1);
    // imshow("After LBP operator", lbpres);
    // imwrite("/Users/calvinlee/eigenfaces/output/Original.jpg"  ,col1);
    // imwrite("/Users/calvinlee/eigenfaces/output/LBPresult.jpg"  ,lbpres);
	...
	Mat eigenFacesNorm(trainMatrix.rows, d, CV_32FC1);			// 10340 x d
